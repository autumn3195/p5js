...
function keyPressed() {
  if (keyCode === LEFT_ARROW && lefteye_x > 240) {
    lefteye_x -= 10; // 왼쪽 화살표 키를 누르면 왼쪽으로 이동
    righteye_x -= 10;
  } else if (keyCode === RIGHT_ARROW && righteye_x < 360) {
    lefteye_x += 10; // 오른쪽 화살표 키를 누르면 오른쪽으로 이동
    righteye_x += 10;
  } else if (keyCode === UP_ARROW && lefteye_y > 165 && righteye_y > 165) {
    lefteye_y -= 5; // 위쪽 화살표 키를 누르면 위로 이동
    righteye_y -= 5;
  } else if (keyCode === DOWN_ARROW && lefteye_y < 175 && righteye_y < 175) {
    lefteye_y += 5; // 아래쪽 화살표 키를 누르면 아래로 이동
    righteye_y += 5;
  } else if (key === 's') {
    saveGif('mySketch', 10);
  }
